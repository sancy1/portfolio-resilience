...
- **Additive** (new types, new optional parameters) → minor bump (`0.5.x` → `0.6.0`)
- **Removal, rename, signature change** → major bump (`0.x` → `1.0`, `1.x` → `2.0`)

## Stability tier

**Every type listed here is stable as of v0.5.0.** There are no experimental or
...
| `Configuration/` | 9 | Options and builder |
...
| `Policies/` | 4 | Retry, timeout, circuit, composite |
| `Sinks/` | 6 | Log and metric sinks |

**Total: 47 public types.**
---
...
| `PolicyDefinition` | class | A single policy: `Retry`, `Circuit`, `Timeout`, `Fallback`, `Logging` |
...
| `ResilienceEventType` | enum | `CallStarted`, `RetryAttempted`, `CallSucceeded`, `CallFailed`, `CircuitOpened`, `CircuitClosed`, `CircuitHalfOpened`, `FallbackUsed`, `TimeoutBreached` |
...
| `CompositePolicyBuilder` | sealed class | Chains retry → circuit → timeout |
...
|--------|---------|--------------|
| Bug fix, no API change | Fix jitter bounds | `0.5.0` → `0.5.1` |
| New optional parameter | Add `bool x = false` to a method | `0.5.0` → `0.6.0` |
| New type, new interface member | Add `IFoo` | `0.5.0` → `0.6.0` |
